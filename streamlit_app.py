...
with st.sidebar:
    ...
    upload=st.file_uploader('Upload one Excel workbook',type=['xlsx'],help='Use the supplied input workbook; preserve the named tabs and headers.')
    ...
    st.caption('No upload? The synthetic coatings example is ready.')
...
raw=upload.getvalue() if upload is not None else b'SYNTHETIC_EXAMPLE_V1'
...
st.markdown(dataset_strip(data,upload.name if upload is not None else 'Synthetic coatings example'),unsafe_allow_html=True)
...
with overview:
    ...
    with right:st.plotly_chart(stage_chart(result),width='stretch',config={'displaylogo':False},key='ov_stages')
    st.caption(f'{result["controls"]["runs"]} paired paths × {result["controls"]["horizon_weeks"]} weeks. Shaded P10–P90 is a scenario range. The owned-stock chart includes on-hand and ordered pipeline under the model’s ownership assumption.')
...
with d1:
    ...
    st.plotly_chart(confidence_chart(result),width='stretch',config={'displaylogo':False},key='classification_conf')
    show_table(result['classification'],height=350)
    sku=st.selectbox('Inspect a SKU',data['Products'].sku.tolist())
    cr=result['classification'].set_index('sku').loc[sku]
    st.info(f'{cr.product_name} · {cr.stage} · {cr.confidence} confidence. {cr.decision_reason}')
...
with d2:
    ...
    st.plotly_chart(policy_chart(result),width='stretch',config={'displaylogo':False},key='policy_targets')
    locations=['All locations']+sorted(result['policy'].location.unique())
    location=st.selectbox('Stocking location',locations)
    selected=result['policy'] if location=='All locations' else result['policy'].loc[lambda p:p.location.eq(location)]
    show_table(selected,height=360)
    with st.expander('Batch expiry exposure'):
        show_table(result['batch_risk'],height=330)
    with st.expander('Screened network transfer candidates'):
        ...
        show_table(result['transfers'],height=280)
    with st.expander('Exit obligations · dated coverage'):
        ...
        show_table(result.get('exit_coverage',pd.DataFrame()),height=280)
with d3:
    ...
    with st.expander('Local service floors and commitments',expanded=True):
        show_table(result.get('service_evidence',pd.DataFrame()),height=280)
